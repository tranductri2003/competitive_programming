...
# Dòng đầu tiên chứa hai số nguyên  và  (, ), lần lượt là số lượng quốc gia và số sự kiện mà Phát đã giảng cho em gái.

# Mỗi dòng trong  dòng tiếp theo mô tả các sự kiện theo mô tả sau:

# 1 u v  – mô tả sự kiện quốc gia  và  (, ) mở tuyến đường hàng không qua lại với nhau. Dữ liệu đảm bảo trước đó quốc gia  và  chưa thiết lập đường hàng không với nhau.

# 2 x y  – mô tả sự kiện quốc gia  và  (, ) kí hiệp ước hợp tác với nhau trong lĩnh vực công nghệ. Dữ liệu đảm bảo trước đó quốc gia  và  chưa kí hiệp ước hợp tác với nhau.

# 3 k  – mô tả sự kiện quốc gia  () có phát kiến công nghệ mới và gửi thư mời các quốc gia đã có hiệp ước với quốc gia  trước đó đến quốc gia  để tham gia hội thảo về công nghệ. Dữ liệu đảm bảo có ít nhât một sự kiện loại này.
n,q=list(map(int,input().split()))
g=Graph()

hiepuoc=defaultdict(list)
access=defaultdict(lambda:defaultdict(lambda:-1))
for i in range(q):
    a=list(map(int,input().split()))
    if a[0]==1:
        g.addEdge(a[1],a[2])
    elif a[0]==2:
        hiepuoc[a[1]].append(a[2])
        hiepuoc[a[2]].append(a[1])
    else:
        path=g.BFS(a[1])
        for num in path:
            access[a[1]][num]=1
            access[num][a[1]]=1
        res=0
        for country in hiepuoc[a[1]]:
            if access[a[1]][country]==1:
                res+=1
        print(res)
