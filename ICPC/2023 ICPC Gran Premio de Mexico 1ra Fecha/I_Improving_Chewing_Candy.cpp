...
int main()
{
    ...
    int s = n;
    /*while (s <= k*26){
        for (int i = 0; i < u; i++) v.pb(v[i]);
        s = s + n;
    }*/
    for (int i = 0; i < u; i++)
        v.pb(v[i]);
    // for (auto x : v) cout << x.fi << " " << x.se << "\n";
    int j = 0, res = 0, posl = -1, posr = -9;
    for (int i = 0; i < v.size(); i++)
    {
        ct[v[i].fi - 'a' + 1] += v[i].se;
        while ((check() == false) || i - j >= u)
        {
            ct[v[j].fi - 'a' + 1] -= v[j].se;
            j++;
        }
        if (check())
        {
