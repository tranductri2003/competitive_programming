...
for _ in range(k):
    ...
    soLuongTrung =0
    trung = defaultdict(lambda: False)
    for xTemp, yTemp, zTemp in data:
        ok =[]
        if x==xTemp:
            ok.append(x)
        elif x<xTemp and x==0:
            ok.append(xTemp)
        elif x>xTemp and xTemp==0:
            ok.append(x)

        if y==yTemp:
            ok.append(y)
        elif y<yTemp and y==0:
            ok.append(yTemp)
        elif y>yTemp and yTemp==0:
            ok.append(y)
            
        if z==zTemp:
            ok.append(z)
        elif z<zTemp and z==0:
            ok.append(zTemp)
        elif z>zTemp and zTemp==0:
            ok.append(z)


        if len(ok)==3 and trung[(ok[0], ok[1], ok[2])]==False:
            soLuongTrung+=1
            trung[(ok[0], ok[1], ok[2])]=True
    data.append((x,y,z))
    res+=n-soLuongTrung
print(res)
