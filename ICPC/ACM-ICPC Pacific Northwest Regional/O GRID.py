...
for i in range(1,m+1):
    dayso=input()
    for j in range(1,n+1):
        matrix[i][j]=int(dayso[j-1])

"""for i in range(1,m+1):
    print(matrix[i][1:])"""
def sosangtoado(n,collumn):
    if n%collumn==0:
        hoanhdo=collumn
    else:
        hoanhdo=n%collumn
    ...
    if n%collumn==0:
        ...
        tungdo=n//collumn+1
    return tungdo,hoanhdo

def toadosangso(tungdo,hoanhdo,cot):
    so=(tungdo-1)*cot+hoanhdo
    return so
...
while queue!=[]:
    u=queue[0]
    queue.pop(0)
    tungdou,hoanhdou=sosangtoado(u,n)

    k=matrix[tungdou][hoanhdou]
    if k==0:
        pass
    else:
        giatri=dp[tungdou][hoanhdou]
        
        if hoanhdou-k>=1 and giatri+1<dp[tungdou][hoanhdou-k]:
            dp[tungdou][hoanhdou-k]=giatri+1
            queue.append(toadosangso(tungdou,hoanhdou-k,n))
            
        if hoanhdou+k<=n and giatri+1<dp[tungdou][hoanhdou+k]:
            dp[tungdou][hoanhdou+k]=giatri+1
            queue.append(toadosangso(tungdou,hoanhdou+k,n))     
        
        if tungdou-k>=1 and giatri+1<dp[tungdou-k][hoanhdou]:
            dp[tungdou-k][hoanhdou]=giatri+1
            queue.append(toadosangso(tungdou-k,hoanhdou,n)) 


        if tungdou+k<=m and giatri+1<dp[tungdou+k][hoanhdou]:
            dp[tungdou+k][hoanhdou]=giatri+1
            queue.append(toadosangso(tungdou+k,hoanhdou,n))       
