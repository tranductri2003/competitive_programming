...
n,k=get_ints() #n: số lần thả, k: số tầng

safe=[]
broken=[]
for i in range(n):
    tang,trangthai=input().split()
    if trangthai=="SAFE":
        safe.append(int(tang))
    else:
        broken.append(int(tang))

safe=list(set(safe))
broken=list(set(broken))


tang_thap_nhat_ma_bi_vo=2
tang_cao_nhat_ma_khong_bi_vo=k-1

if safe!=[]:
    tang_thap_nhat_ma_bi_vo=max(safe)+1
    
if broken!=[]:
    tang_cao_nhat_ma_khong_bi_vo=min(broken)-1

print (f"{tang_thap_nhat_ma_bi_vo} {tang_cao_nhat_ma_khong_bi_vo}")
