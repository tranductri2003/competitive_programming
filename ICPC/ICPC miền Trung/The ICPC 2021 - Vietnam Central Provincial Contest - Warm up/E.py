...
maxa=max(a)
mangsonguyento=rwh_primes1v1(maxa+1)
mangcheck=[False]*(maxa+1)

for i in mangsonguyento:
    mangcheck[i]=True

current=0
soluongsonguyento=[0]*n
for i in range(0,n):
    so=a[i]
    if mangcheck[so]==True:
        current+=1
        soluongsonguyento[i]=current
    else:
        soluongsonguyento[i]=current

res=0
soluongsonguyento.insert(0,0)
for i in range(1,n+1):
    for j in range(i,n+1):
        soluongsonguyentotrongdoanij=soluongsonguyento[j]-soluongsonguyento[i-1]            
        soluongsobinhthuongtrongdoanij=j-i+1-soluongsonguyentotrongdoanij
        if soluongsonguyentotrongdoanij>=soluongsobinhthuongtrongdoanij:
            res+=1
print(res)
