...
def find_six_prime_factors(N):
    prime_factors = []
    prime_candidate = 2

    while len(prime_factors) < 6 and prime_candidate <= N:
        if N % prime_candidate == 0 and is_prime(prime_candidate):
            prime_factors.append(prime_candidate)
            N //= prime_candidate
        else:
            prime_candidate += 1

    return prime_factors if len(prime_factors) == 6 else None

N = int(input())
result = find_six_prime_factors(N)
...
data = rwh_primes1v1(10**6)
print(data[-1], data [-2])
print(5*7*11*13*999983*999979)