...
turnOnIndex = defaultdict(lambda: -1)
turnOnNumber = defaultdict(lambda: -1)
pos = defaultdict(lambda: -1)
data = []
for _ in range(n):
    data.append(int(input()))
    # data.append(randint(1, k))

fromLeft = Counter()
fromRight = Counter(data)
res = []
for num in data:
    fromRight[num] -= 1
    if fromLeft[num] > 0:  # có rồi thì bỏ qua
        continue
    else:  # Chưa có thì kiểm tra xem nếu như nó lớn hơn thì cập nhật
        while len(res) > 0 and res[-1] > num and (fromRight[res[-1]] > 0 or fromLeft[res[-1]] > 1):
            fromLeft[res[-1]] -= 1
            res.pop()
...
print(*res)
# for i in range(n):
#     if turnOnNumber[data[i]] == -1:
#         turnOnNumber[data[i]] = 1
#         pos[data[i]] = i
#         turnOnIndex[i] = 1
#     else:
#         prePos = pos[data[i]]
...
#             continue
#         else:
#             if data[i] > data[prePos+1]:
#                 turnOnIndex[prePos] = -1
#                 pos[data[i]] = i
#                 turnOnIndex[i] = 1
