...
"""
#!Cách dùng Graph phải dùng thêm cả fastIO
from collections import defaultdict
...
class Graph:
    ...
    def connectedComponentsBFS(self,numVertex,check):
        visited=defaultdict(lambda:0)
        stack=[]
        path=[]
        for i in range(1,numVertex+1):
...
#Nếu 2 từ có chung 1 chữ => liên kết với nhau
#Nếu 2 từ cùng nối chung 1 từ => liên kết với nhau

# the answer to the problem is the number of connected components in this bipartite graph.
def strToNum(a):
    return ord(a)-96
...
for edge in data:
    ...
    if len(edge)==1:
        ...
        for i in range(len(edge)-1):
            str1=strToNum(edge[i])
            str2=strToNum(edge[i+1])
            check[str1]=True
            check[str2]=True
            g.addEdge(str1, str2)  


connect=g.connectedComponentsBFS(26,check)
res=len(connect)
...
for i in range(n):
    ...
    for i in range(len(s)-1):
        g.Union( strrToNum(s[i]) , strrToNum(s[i+1]) )

for num in past:
    for i in range(len(num)-1):
        g.Union( strrToNum(num[i]) , strrToNum(num[i+1]) )   

res=[]
for strr in data:
    res.append(g.parent[strrToNum(strr)])
