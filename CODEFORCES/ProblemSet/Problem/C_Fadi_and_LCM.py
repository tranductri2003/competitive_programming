...
if x==1:
    ...
    data=[1]
    for num in temp:
        data.append(num**a.count(num))


    tapcon = [x for x in powerset(data)]
    tapcon.sort()
    
    xoa=[]
    for i in range(0,len(tapcon)):
        if tapcon[i]==[] or len(tapcon[i])==len(data):
            xoa.append(i)
    ...
    ketqua=[]    
    for i in range(len(tapcon)):
        temp=[]
        for num in data:
            if num not in tapcon[i]:
                temp.append(num)
        ...
        for num in temp:
            thu2*=num
        ketqua.append((thu1,thu2))
    
    
    kiemtra=[]
    for num in ketqua:
        kiemtra.append(max(num))
    vitri=kiemtra.index(min(kiemtra))
    
    ress=[]
    for num in ketqua[vitri]:
        if "." in str(num):
            ress.append(str(num)[:-2])
        else:
            ress.append(str(num))
    print(*ress)
