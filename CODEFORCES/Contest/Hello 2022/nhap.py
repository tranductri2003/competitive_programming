...
for test in range(testcase):
    ...
    res=0
    for i in range(0,n):
        l[i],r[i],c[i]=get_ints()
        if l[i]==r[i]:
            if i==0:
                sonhonhattrai=solonnhatphai=l[i]
                block.append(l[i])
                print(c[i])
                res=c[i]
            else:
                if (l[i]<sonhonhattrai or l[i]>solonnhatphai) and l[i] not in block:
                    print(res+c[i])
                    res=res+c[i]
                    block.append(l[i])
                else:
            ...
            if l[i]<=sonhonhattrai or r[i]>=solonnhatphai:
                if l[i]<sonhonhattrai and r[i]>solonnhatphai:
                    sonhonhattrai=l[i]
                    solonnhatphai=r[i]
                    cantrai=i
                    canphai=i
                    print(c[i])
                    res=c[i]
                elif l[i]==sonhonhattrai and r[i]>solonnhatphai:
                    solonnhatphai=r[i]
                    canphai=i
                    if c[i]<c[cantrai]:
                        cantrai=i
                    ...
                    cantrai=i
                    sonhonhattrai=l[i]
                    print(c[cantrai]+c[canphai])
                    res=c[cantrai]+c[canphai]
                elif l[i]>sonhonhattrai and r[i]>solonnhatphai:
                    canphai=i
                    solonnhatphai=r[i]
                    print(c[cantrai]+c[canphai])
                    res=c[cantrai]+c[canphai]
                elif r[i]==solonnhatphai and l[i]>sonhonhattrai:
                    if c[i]<c[canphai]:
                        canphai=i
                        if r[cantrai]!=r[i]:
                            print(c[canphai]+c[cantrai])
                            res=c[cantrai]+c[canphai]
                        else:
                            print(res)
                    else:
