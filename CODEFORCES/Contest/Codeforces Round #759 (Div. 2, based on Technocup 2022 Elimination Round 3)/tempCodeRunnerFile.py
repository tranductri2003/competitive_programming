...
for test in range(testcase):
    ...
    for i in range(0,n):
        if a[i]>=0:
            mangduong.append(a[i])
        else:
            mangam.append(abs(a[i]))
    mangduong.sort()
    mangam.sort()
    res=0
    maxam=-10**19
    chieudaimangam=len(mangam)
    if chieudaimangam>0:
        maxam=max(mangam)
    chieudaimangduong=len(mangduong)
    maxduong=-10**19
    if chieudaimangduong>0:
        maxduong=max(mangduong)
    if chieudaimangam==0 or maxduong>maxam:
        #end by mangduong
        for i in range(len(mangam)-1,-1,-k):
            res+=mangam[i]*2
            
        for i in range(len(mangduong)-1,-1,-k):
            if i==len(mangduong)-1:
                res+=mangduong[i]
            else:
                res+=mangduong[i]*2
            
  
    elif chieudaimangduong==0 or maxam>maxduong:
        #end by mangam
        for i in range(len(mangduong)-1,-1,-k):
            res+=mangduong[i]*2
