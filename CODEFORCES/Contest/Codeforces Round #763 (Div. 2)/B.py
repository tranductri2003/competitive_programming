...
for test in range(testcase):
    ...
    for i in range(0,n):
        ...
        if l==r:
            res[i]=l
            check[l]=1
        khoangcach.append(abs(l-r))
    khoangcachdasapxep=sorted(khoangcach)
    for i in range(0,len(khoangcachdasapxep)):
        if khoangcachdasapxep[i]!=0:
            for j in range(0,len(khoangcach)):
                if khoangcach[j]==khoangcachdasapxep[i]:
                    vitri=j
                    khoangcach[j]=-1
                    break
            if check[l_[vitri]]==1 and check[r_[vitri]]==1:
                for j in range(1,n+1):
                    if check[j]==0 and l_[vitri]<j<r_[vitri]:
                        res[vitri]=j
                        check[j]=1
                        break
            elif check[l_[vitri]]==1:
                res[vitri]=r_[vitri]
                check[r_[vitri]]=1
            else:
                res[vitri]=l_[vitri]
