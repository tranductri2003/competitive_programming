...
for test in range(testcase):
    ...
    if sum(a)<=k:
        ...
        sonhonhat=a[0]
        for num in a:
            mangkhoangcach.append(num-sonhonhat)
        prefixtru=[0]*(n)
        prefixtru[n-1]=mangkhoangcach[n-1]
        for i in range(n-2,-1,-1):
            prefixtru[i]=prefixtru[i+1]+mangkhoangcach[i]
        if prefixtru[0]>=cantru:
            res=1
            for i in range(n-1,-1,-1):
                if prefixtru[i]>=cantru:
                    break
                res+=1
            print(res)
        else:
            res=1
            solancantrusodau=((cantru-prefixtru[0])//(n-1)+1)//(1+1//(n-1))
            for i in range(n-1,0,-1):
                prefixtru[i]=prefixtru[i]+solancantrusodau*res
                if prefixtru[i]+solancantrusodau>=cantru:   
                    break
                res+=1
            prefixtru[0]=prefixtru[0]+solancantrusodau*(n-1)
            res+=solancantrusodau
            print(res)
