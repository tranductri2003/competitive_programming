...
#Phép cộng theo cột ở các cột có vị trí chẵn là đúng, vị trí lẻ là đúng
for test in range(0,testcase):
    s="0"+inputf()   #Phải thêm ký tự 0 ở đầu để cho lệnh b=int(s[1::2]) vẫn thực hiện được dù có 1 ký tự
    
    a=int(s[0::2])  #Lấy giá trị các phần tử trong s có vị trí là số chẵn
    b=int(s[1::2])  #Lấy giá trị các phần tử trong s có vị trí là số lẽ
    
    res=(a+1)*(b+1)-2 #Số 22 có 23 (22-0+1) cách tạo ra từ tổng của 2 số. 
    #Vậy số cách để tạo ra số ban đầu là (a+1)*(b+1)
    #Phải trừ 2 là vì 2 trường hợp khi cả hàng trên đều bằng 0 hoặc cả hàng dưới đều bằng 0.
    #Ví dụ:
    """
    14  ri thì đúng 14
    14              12
