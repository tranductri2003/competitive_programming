...
for _ in range(t):
    ...
    ROW, COLLUMN = [[] for _ in range(m)], [[] for _ in range(n)]
    ...
    if valid == False:
        ...
        for j in range(m):
            for k in range(0, len(ROW[j]), 2):
                i1, i2 = ROW[j][k], ROW[j][k + 1]
                matrix[i1][j] = 'W'
                matrix[i2][j] = 'B'
                matrix[i1][j + 1] = 'B'
                matrix[i2][j + 1] = 'W'
        ...
        for i in range(n):
            for k in range(0, len(COLLUMN[i]), 2):
                j1, j2 = COLLUMN[i][k], COLLUMN[i][k + 1]
                matrix[i][j1] = 'W'
                matrix[i][j2] = 'B'
                matrix[i + 1][j1] = 'B'
                matrix[i + 1][j2] = 'W'
