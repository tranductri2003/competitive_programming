...
for i in range(n):
    sumPart.append([])
for i in range(n):
    t[i],z[i],y[i]=get_ints()
    sumTime.append(t[i])
    for j in range(z[i]):
        sumTime.append(t[i])
        sumPart[i].append(t[i])

"""sumTime.sort()
res=sum(sumTime[0:m])
print(res)"""

#Tính thời gian mỗi người
...
for i in range(m):
    ...
    for j in range(n):
        if sumPart[j]==[]:
            ...
            if dabo[j]%z[j]==0 and dabo[j]!=0:
                sumCur.append(sumPart[j][0]+y[j])
            else:
                sumCur.append(sumPart[j][0])
    minn=min(sumCur)
    pos=sumCur.index(minn)
    sumPart[pos].pop(0)
    dabo[pos]+=1
    resPos[pos]+=1
...
for i in range(0,n):
    resPart=resPos[i]*t[i]
    if resPos[i]>=1:
        resPart+=(resPos[i]-1)//z[i] *y[i]
    res=max(res,resPart)
print(res)
print(*resPos)