...
"""
Hướng làm
Tìm số số đặc biệt với mỗi số lớn nhất tận cùng chỉ bằng các số 9,99,999... trừ cho số nhỏ nhất 
có tận cùng 9, 99,999,9999 chia cho 10,100,1000,10000 và cộng 1.
Kết quả cộng bằng r-l
"""
testcase=int(input())
for test in range(testcase):
    ...
    #Tìm cận dưới
    canduoi=[]
    for num9 in range(1,len(str(r))+1):
        newNum=(l//pow(10,num9)+1)*pow(10,num9)-1
        if l<=newNum<=r:
            canduoi.append(newNum)

    #Tìm cận trên
    cantren=[]
    for num9 in range(1,len(str(r))+1):
        newNum=r//pow(10,num9)*pow(10,num9)-1
        if l<=newNum<=r:
            cantren.append(newNum)
    
    res=0
    for i in range(0,len(cantren)):
        res+=(cantren[i]-canduoi[i])//pow(10,i+1)+1
    res+=r-l
    print(res)
