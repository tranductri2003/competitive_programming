...
for test in range(testcase):
    ...
    if socuu==[]:
        ...
        prefixSum=[0]
        temp=0
        for i in range(0,socuu):
            temp+=vitricuu[i]
            prefixSum.append(temp)
        
        for vitrihientai in range(0,socuu):
            vitribatdau=vitricuu[vitrihientai]-vitrihientai
            vitriketthuc=vitribatdau+socuu-1
            bentraithutudung=(vitricuu[vitrihientai]-vitribatdau+1)*(vitricuu[vitrihientai]+vitribatdau)/2
            traitong=prefixSum[vitrihientai+1]-prefixSum[0]
            
            phaitong=prefixSum[socuu]-prefixSum[vitrihientai+1]
            benphaithutudung=(vitriketthuc-(vitricuu[vitrihientai]+1)+1)*(vitriketthuc+(vitricuu[vitrihientai]+1))/2
            
            giatri=bentraithutudung-traitong+phaitong-benphaithutudung
            res=min(res,giatri)
        if res==10**9:
            print(0)
        else:
