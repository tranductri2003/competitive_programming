...
for _ in range(t):
    ...
    trenCo = []
    duoiCo = []
    checkTrenCo = defaultdict(lambda: False)
    checkDuoiCo = defaultdict(lambda: False)
    ...
    for i in range(n):
        if dem[a[i]] == 0:
            ...
            dem[a[i]] += 1
            trenCo.append(a[i])
            checkTrenCo[a[i]] = True
        elif dem[a[i]] == 1:
            duoi[i+1] = a[i]
            dem[a[i]] += 1
            duoiCo.append(a[i])
            checkDuoiCo[a[i]] = True
        else:
            print("NO")
            break
    else:
        ...
        for i in range(1, n+1):
            if checkDuoiCo[i] == False:
                duoiThieu.append(i)
            if checkTrenCo[i] == False:
                trenThieu.append(i)

        trenCo = sorted(trenCo, reverse=True)
        duoiThieu = sorted(duoiThieu, reverse=True)

        dataDuoi = defaultdict(list)
        for i in range(len(trenCo)):
            if duoiThieu[i] > trenCo[i]:
                stop = True
            else:
                dataDuoi[trenCo[i]].append(duoiThieu[i])
        ...
        if stop == True:
            ...
            for i in range(n):
                if duoi[i+1] == 0:
                    duoi[i+1] = dataDuoi[tren[i+1]][0]
                    dataDuoi[tren[i+1]].pop(0)

            trenThieu = sorted(trenThieu, reverse=True)
            duoiCo = sorted(duoiCo, reverse=True)

            dataTren = defaultdict(list)
            for i in range(len(duoiCo)):
                if trenThieu[i] > duoiCo[i]:
                    stop = True
                else:
                    dataTren[duoiCo[i]].append(trenThieu[i])
            for i in range(n):
                if tren[i+1] == 0:
                    tren[i+1] = dataTren[duoi[i+1]][0]
                    dataTren[duoi[i+1]].pop(0)
