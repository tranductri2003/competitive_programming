...
for i in range(n):
    c[i],d[i],h[i] = list(map(int,input().split()))
...
H=[0]*m
for i in range(m):
    D[i],H[i] =list(map(int,input().split()))
...
for i in range(m):
    tien=10**9
    # for j in range(n):
    #     solancan=1/((h[j]/D[i])/(H[i]/d[j]))
    #     if solancan%1==0:
    #         solancan+=1
    #     else:
    #         solancan=math.ceil(solancan)
    #     tien=min(tien,solancan*c[j])
    # if tien>C:
    #     res[i]=-1
    # else:
    #     res[i]=tien
    quaivat=D[i]*H[i]
    pos=upper_bound(ketqua,quaivat)
    if quaivat>ketqua[pos]:
        if quaivat%ketqua[pos]==0:
            tien=(quaivat//ketqua[pos])+1
        else:
            tien=math.ceil(quaivat//ketqua[pos])
        tien*=c[pos]
    else:
        tien=c[pos]
