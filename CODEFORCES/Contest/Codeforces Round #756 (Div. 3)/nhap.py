...
for test in range(testcase):
    n=int(inputf())     #Số đỉnh
    b=get_ints()      #Cho tổ tiên lần lượt các đỉnh => Định hình cây
    b.insert(0,0)
    p=get_ints()   #Thư tự các đỉnh được sắp xếp theo chiều tăng dần khoảng cách từ gốc đến đỉnh đó
    p.insert(0,0)
    ...
    for i in range(1,n+1):
        if b[i]==i:
            root=i
            break
    if root==-1 or root!=p[1]:
        print(-1)
    else:
        currentmax=0
        res=[0]*(n+1)  #Khoảng cách từ tổ tiên của đỉnh i đến đỉnh i
        reschuan=[0]*(n+1)
        p.pop(0)
        for num in p:
            if num==root:
                ...
                res[num]=max(res[b[num]]+1,currentmax+1)
                currentmax=max(currentmax,res[num])
                reschuan[num]=res[num]-res[b[num]]
