...
for test in range(testcase):
    ...
    if b[p[1]]!=p[1]:
        ...
        dist=[-1]*(n+1)  #Khoảng cách từ gốc đến đỉnh i
                        #-1 vì để đề phòng trường hợp đỉnh i nối với đỉnh i khác mà đỉnh i chưa tồn tại
        dist[p[1]]=0
        for i in range(2,n+1):
            if dist[b[p[i]]]==-1:    #Nếu tổ tiên của nó chưa tồn tại 
                print(-1)
                stop=True
                break
            else:
