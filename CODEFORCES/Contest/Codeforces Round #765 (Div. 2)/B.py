...
for test in range(testcase):
    ...
    if len(set(a))==n:
        ...
        b=list(set(a))
        sophantukhacnhau=len(b)
        vitriso=[]
        for i in range(0,max(a)+1):
            vitriso.append([])
        
        for i in range(0,n):
            vitriso[a[i]].append(i)

        res=0
        for i in range(0,sophantukhacnhau):
            currentNum=b[i]
            if len(vitriso[currentNum])>=2:
                for j in range(0,len(vitriso[currentNum])-1):
                    res=max(res,vitriso[currentNum][j]+n-vitriso[currentNum][j+1])
        print(res)
