...
#Mỗi loại sản phẩm có không quá k đơn vị
#W là khối lượng giới hạn
from collections import defaultdict
if k!=1:
    w=[0]*(n+1)
    v=[0]*(n+1)
    ...
    for i in range(n):
        ...
        uutien[i]=v[i]/w[i]
    uutien = sorted(uutien.items(), key=lambda x: x[1], reverse=True)
    res=0
    for i in range(0,n):
        if W<w[uutien[i][0]]:
            break
        else:
            solanmua=min(k,W//w[uutien[i][0]])
            res+=v[uutien[i][0]]*solanmua
            W-=w[uutien[i][0]]*solanmua
    print(res)
else:
    if W>=10**5:
        INF=10**10
